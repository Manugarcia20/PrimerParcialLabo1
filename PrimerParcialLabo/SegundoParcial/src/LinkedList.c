...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
	...
	Node* prevNode;
	    ...
	    if(this != NULL && nodeIndex >= 0 && nodeIndex <= tamList)
	    {
	    	...
	    	if(nodeIndex == 0) /// si lo quiero agregar en la primera posicion
	    	{
    ...
    if(this!= NULL && pFunc!= NULL && (order == 1 || order ==0)){
    	...
    	for(int i = 0; i<ll_len(this)-1;i++){
    		...
    		for(int j = i+1;j<ll_len(this);j++){
    			...
    			if ((order == 1 && pFunc(pElement1,pElement2) == 1) /// si el orden es 1 y la funcion criterio que le paso me da 1
    			|| (order == 0 && pFunc(pElement1,pElement2)  == -1))/// si el orden es 0 y la funcion criterio que le paso me da -1
    			{
 ...
 * @param  pFunc Funcionq que le indicara que debe filtrar
	...
	if (this != NULL && *pFunc != NULL) {
		...
		if (newLinkedList != NULL) {
			...
			for (int i = 0; i < tamanio; i++) {
				...
				if (pFunc(pElement) == 1) {  /// si pFunc devuelve 1
					ll_add(newLinkedList, pElement); ///los agrego a la lista filtrada.
				}
			}
		}
	}
...
LinkedList* ll_map(LinkedList* this, void* (*pFunc)(void*)){
	...
	if(this!=NULL && *pFunc!=NULL){
		...
		for (int i = 0; i < tamanio; i++) {
			...
			pElement = ll_get(this, i); /// traigo elementos en cada posicion
			pElement = pFunc(pElement);

			if(pElement!=NULL){
				ll_add(listaMapeada, pElement); ///los agrego a la lista filtrada.
			}
		}
	}
